# Generation of Fig8_5 with CairoMakie for CS2_1.jl
...
for row in 1:pp
    for col in 1:mm
        ...
        for k = 1:N
            lines!(axs[row, col], tout, y[k][:,row,inputs[col]], color = (:blue, 3))
        end
        xlims!.(axs[row, col], low = 0, high = 10) 
        ylims!.(axs[row, col], low = -0.1, high = 2)
    end
end
