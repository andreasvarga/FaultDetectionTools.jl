...
sysf = fdimodset(dss([Gu Gf Gw],minimal = true), c = 1:mu, 
                 f = mu .+ (1:mf), n = (mu+mf) .+ (1:mw));

# call of AFDSYN with the options for stability degree -3 and the synthesis 
# of a scalar output filter
Q, Rfw, info = afdsyn(sysf; atol, minimal = false, 
                      nullspace = false, smarg = -2, sdeg = -3, 
                      rdim = 1, HDesign = [1 1]); 

println("Q = $(dss2rm(Q.sys; atol))")
println("Rf = $(dss2rm(Rfw.sys[:,Rfw.faults]; atol))")
println("Rw = $(dss2rm(Rfw.sys[:,Rfw.noise]; atol))")
gap = fdif2ngap(Rfw)[1]; println("gap = $gap")
β = fdiscond(Rfw)[1];    println("scond = $β")
mmperf = fdimmperf(Rfw); println("mmperf = $mmperf")
